...
def check_tcp(cfg):
    ...
    try:
        ...
        return CAIDO, None, f"no se pudo conectar a {host}:{puerto}: {error}"

    latencia_ms = (time.monotonic() - inicio) * 1000
    if latencia_ms >= umbral_degradado:
        return DEGRADADO, latencia_ms, f"conexión lenta: {latencia_ms:.0f}ms"
    return OK, latencia_ms, f"conexión OK en {latencia_ms:.0f}ms"
